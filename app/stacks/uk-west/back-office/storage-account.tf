...
resource "azurerm_private_endpoint" "functions_storage_private_endpoint" {
  name                = "${local.org}-pe-st-funcbo-${local.resource_suffix}"
  location            = azurerm_resource_group.back_office_stack.location
  resource_group_name = azurerm_resource_group.back_office_stack.name
  subnet_id           = azurerm_subnet.back_office_ingress.id

  private_dns_zone_group {
    name                 = "${local.org}-pdns-${local.service_name}-funcbo-${var.environment}"
    private_dns_zone_ids = [data.azurerm_private_dns_zone.storage.id]
  }

  private_service_connection {
    name                           = "${local.org}-psc-funcbo-${local.resource_suffix}"
