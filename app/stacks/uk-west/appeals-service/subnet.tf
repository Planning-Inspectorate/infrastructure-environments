...
resource "azurerm_virtual_network_peering" "appeals_service_to_back_office" {
  ...
  remote_virtual_network_id = data.azurerm_virtual_network.appeals_bo.id #TEST environment
...
resource "azurerm_virtual_network_peering" "back_office_to_appeals_service" {
  ...
  remote_virtual_network_id = data.azurerm_virtual_network.staging_afo_vnet.id
